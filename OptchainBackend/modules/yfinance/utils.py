...
def parse_quotes(data, tz=None, toDict=False):
    ...
    if toDict:
        ...
        for i in range(len(timestamps)):
            timestamp = _datetime.datetime.fromtimestamp(timestamps[i])
            
            if tz:
                if isinstance(tz,str):
                    timestamp = _pytz.timezone(tz).localize(timestamp)
                else:
                    timestamp = timestamp.replace(tzinfo=tz)

            quotes[timestamp] = {"Open": opens[i],
                "High": highs[i],
                "Low": lows[i],
                "Close": closes[i],
